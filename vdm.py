...
class LightVDM(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if batch_idx in indices:
            ...
            ax.flat[3].set_title("Density")
            k, P, N = power(x[0])
            ax.flat[4].loglog(k.cpu(), P.cpu()*(k_true.cpu()**2), label="True DM", color='#e98d6b')
            k, P, N = power(sample[0])
            ax.flat[4].loglog(k.cpu(), P.cpu()*(k_true.cpu()**2), label="Sampled DM", color='#b13c6c')
            ax.flat[4].legend(fontsize=12)
            ax.flat[4].set_xlabel('k',fontsize=15)
            ...
            k, P, N = power(x)
            ax.flat[5].loglog(k.cpu(), P.cpu()*(k_true.cpu()**2), label="True DM", color='#e98d6b')
            k, P, N = power(sample)
            ax.flat[5].loglog(k.cpu(), P.cpu()*(k_true.cpu()**2), label="Sampled DM", color='#b13c6c')
