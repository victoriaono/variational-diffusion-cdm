...
from .utils import FixedLinearSchedule, LearnedLinearSchedule, kl_std_normal, power
...
class LightVDM(LightningModule):
    ...
    def validation_step(self, batch: Tuple, batch_idx: int) -> Tensor:
        ...
        if batch_idx == 0:
            ...
            axes.flat[0].imshow(conditioning[0,0,:,:].cpu())
            axes.flat[1].imshow(x[0,0,:,:].cpu())
            axes.flat[2].imshow(sample[0,0,:,:].cpu())
            axes.flat[0].set_title("in field")
            axes.flat[1].set_title("tgt field")
            axes.flat[2].set_title("Sampled field")
            
            #--------
            _ = axes.flat[3].hist(x[0].cpu().numpy().flatten(),bins=np.linspace(-4,4,50),histtype="step",label='in field')
            _ = axes.flat[3].hist(sample[0].cpu().numpy().flatten(),bins=np.linspace(-4,4,50),histtype="step",label='Sampled field')
            
            #--------
            k,P,N = power(x[0:1])
            axes.flat[4].plot(k.cpu(),P.cpu(),label='in field')
            k,P,N = power(sample[0:1])
            axes.flat[4].plot(k.cpu(),P.cpu(),label='Sampled field')
            axes.flat[4].legend(fontsize=15)
            axes.flat[4].set_xscale('log')
            axes.flat[4].set_yscale('log')
            axes.flat[4].set_xlabel('k',fontsize=15)
            axes.flat[4].set_ylabel('Pk',fontsize=15)
            axes.flat[4].set_title("Power")
...
class LightVDM2(LightningModule):
    def __init__(
        self,
        score_model: nn.Module,
        learning_rate: float = 3.0e-4,
...
class LightVDM3(LightningModule):
    def __init__(
        self,
        score_model: nn.Module,
        learning_rate: float = 3.0e-4,
